...
export default function TournamentBracket() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await tournamentApi.getBracket(tid);
      setData(res.data?.bracket || null);
    } catch (e) {
  ...
  if (loading) {
    return (
      <div className="tournament-bracket page-container">
                  ...
                  {(rounds[rk] || []).map((m) => {
                    ...
                    return (
                      <div key={m.id} className="bracket-match">
                        ...
                        <div className={`match-player ${w1 ? 'winner' : ''}`}>
                          {m.player1?.username || '—'}
                        </div>
                        <div className={`match-player ${w2 ? 'winner' : ''}`}>
                          {m.player2?.username || '—'}
                        </div>
                        {(m.status === 'ready' || m.status === 'waiting_ready') && (
                          <div className="bracket-readiness">
                            P1: {m.player1_ready ? '✓' : '…'} · P2: {m.player2_ready ? '✓' : '…'}
                          </div>
                        )}
                      </div>
                    );
